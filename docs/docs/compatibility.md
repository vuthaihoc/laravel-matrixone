...
- **A `DESC` key after a boolean `ORDER BY` key is ignored**: `order by score is null, score desc` returns rows in insertion order. To sort NULLs last, `order by score desc` already puts them last on MatrixOne; for ascending order `order by score is null, score asc` works.
